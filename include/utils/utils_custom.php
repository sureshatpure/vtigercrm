...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="")
{
	...
	if($user_array == null)
	{
		...
		if (empty($status)) {
				...
				if($private == 'private')
				{
					...
					echo $query = "select id as id,user_name as user_name,first_name,last_name from vtiger_users where id=? and status='Active' union select vtiger_user2role.userid as id,vtiger_users.user_name as user_name ,
				...
				else
				{
					...
					// $params = array($status);
					// Commented by J Suresh for changing the query to private
					...
					$query = "select id as id,user_name as user_name,first_name,last_name from vtiger_users where id=".$current_user->id." and status='Active' union select vtiger_user2role.userid as id,vtiger_users.user_name as user_name ,
							  vtiger_users.first_name as first_name ,vtiger_users.last_name as last_name
							  from vtiger_user2role inner join vtiger_users on vtiger_users.id=vtiger_user2role.userid inner join vtiger_role on vtiger_role.roleid=vtiger_user2role.roleid where vtiger_role.parentrole like '".$current_user_parent_role_seq."%' and status='Active' union
							  select shareduserid as id,vtiger_users.user_name as user_name ,
					...
					// commented on 19 Aug 2013 by suresh for listing subordinate users
					/*		  	
					$query="select 
									id as id,
							...
							from 
									vtiger_users 
							where 
									id=".$current_user->id." and status='Active' 
							union 

							SELECT 
									vtiger_user2role.userid as id,
									vtiger_users.user_name as user_name , 
									vtiger_users.first_name as first_name ,
									vtiger_users.last_name as last_name 

							FROM 
									vtiger_user2role inner join vtiger_users on vtiger_user2role.userid = vtiger_users.id 
							WHERE 
									cast( substring( roleid, 2, length( roleid ) -1 ) as binary) >=".$current_user->id."							union
							select 
									shareduserid as id,
									vtiger_users.user_name as user_name , 
									vtiger_users.first_name as first_name ,
									vtiger_users.last_name as last_name 
							from 
									vtiger_tmp_write_user_sharing_per inner 
									join vtiger_users on vtiger_users.id=vtiger_tmp_write_user_sharing_per.shareduserid 
							where 
									status='Active' and 
									vtiger_tmp_write_user_sharing_per.userid=".$current_user->id." and 
									vtiger_tmp_write_user_sharing_per.tabid=".$current_user->id." OR 
									id=12
	...
	/*	if (!empty($assigned_user)) {
		...
		echo"private query ".$query."<br>";
		print_r($params);
		//$result = $db->pquery($query, $params, true, "Error filling in user array: ");
		$result = $db->pquery($query, "", true, "Error filling in user array: ");
