...
if($focus->is_authenticated())
{
	...
	/*extra session variable created by suresh 20-Aug-2013 START*/
	// CUSTOM CODE FOR ADDING AN EXTRA SESSION VARIABLE WITH THE USERS PROFILE ID
	$uId = $focus->id; // Loggedin User ID
	// Get Role ID for User
	$query = 'SELECT roleid from vtiger_user2role where userid=?';
	$res = $adb->pquery($query,array($uId));
	$roleId = $adb->query_result($res, 0, 'roleid');
	// Get Profile ID for User-Role
	$query2 = 'SELECT profileid from vtiger_role2profile WHERE roleid=?';
	$res2 = $adb->pquery($query2,array($roleId));
	$profileId = $adb->query_result($res2, 0, 'profileid');
	// Write an extra SESSION var for identifying
	$_SESSION['profile_id'] = $profileId;
	// END OF CUSTOM CODE
	/*extra session variable created by suresh 20-Aug-2013 END*/
